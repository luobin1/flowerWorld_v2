			...
			<div id="templatemo_main">
					...
					<div class="sidebar_box">
						...
						<div class="content">
							<ul class="sidebar_list">
								...
								<li class="current-li per-li2"><a href="toPersonal.do">个人资料<span>></span></a></li>
				...
				<div id="content" class="right">
					<div class="personal-r f-r"
						style="border: 1px ridge; float: left; height: 920px;">
						<c:if test="${empty comments}">
							<h4>你还没有评论</h4>
							<h3><a href="/flower/getFlowers.do">去添加</a></h3>
						</c:if>
						<c:forEach items="${comments}" var="comment">
							<div
								style="width: 65%; height:175px; border: 0px ridge; float: right;">
								<div class="pcm-top" style="height: 25px;">
									评论内容:
								</div>
								<div class="pcm-info"
									style="border: 1px ridge; width: out; height: 75px; font-size: 15pt; padding: 5px;">
							...
							<div
								style="width: 30%; height: 130px; border: 1px ridge; float: right;">
								<div style="float: right">
									<img alt="" style="width: 100px; height: 100px;"
										src="${comment.flower.getFlowerUrl1() }" />
								</div>
								<div>
									<p>鲜花名称：${comment.flower.getFlowerName() }</p>
									<p>鲜花价格：${comment.flower.getPrice() }</p>
									<p>鲜花材质：${comment.flower.getMaterial() }</p>
								</div>
							</div>
							<br>
						</c:forEach>
						...
						<!--分页-->
						<div align="center" class="paging" style="margin-left: 325px;margin-top: 500px;">
							<font size="2">共 ${page.totalPageCount} 页</font> <font size="2">第
								${page.pageNow} 页</font> <a href="findCommenByLimit.do?pageNow=1&u_id=${user.u_id }">首页</a>
							<c:choose>
								<c:when test="${page.pageNow - 1 > 0}">
									<a href="findCommenByLimit.do?pageNow=${page.pageNow - 1}&u_id=${user.u_id }">上一页</a>
								</c:when>
								<c:when test="${page.pageNow - 1 <= 0}">
									<a href="findCommenByLimit.do?pageNow=1&u_id=${user.u_id }">上一页</a>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${page.totalPageCount==0}">
									<a href="findCommenByLimit.do?pageNow=${page.pageNow}&u_id=${user.u_id }">下一页</a>
								</c:when>
								<c:when test="${page.pageNow + 1 < page.totalPageCount}">
									<a href="findCommenByLimit.do?pageNow=${page.pageNow + 1}&u_id=${user.u_id }">下一页</a>
								</c:when>
								<c:when test="${page.pageNow + 1 >= page.totalPageCount}">
									<a href="findCommenByLimit.do?pageNow=${page.totalPageCount}&u_id=${user.u_id }">下一页</a>
								</c:when>
							</c:choose>
							<c:choose>
								<c:when test="${page.totalPageCount==0}">
									<a href="findCommenByLimit.do?pageNow=${page.pageNow}&u_id=${user.u_id }">尾页</a>
								</c:when>
								<c:otherwise>
									<a href="findCommenByLimit.do?pageNow=${page.totalPageCount}&u_id=${user.u_id }">尾页</a>
								</c:otherwise>
							</c:choose>
						</div>
						<!-- 分页功能 End -->
